...
public class DSNDumpCompareReportGenerator
{

    public DSNDumpCompareReportGenerator()
    {
    }
        ...
        throws Exception
    {
        ...
        ResultSetMetaData resultsetmetadata = resultset.getMetaData();
        for(int i = 0; i < resultsetmetadata.getColumnCount(); i++)
            if(!resultsetmetadata.getColumnTypeName(i + 1).toUpperCase().equals("CLOB") && !resultsetmetadata.getColumnTypeName(i + 1).toUpperCase().equals("BLOB") && !resultsetmetadata.getColumnName(i + 1).toUpperCase().equals("SYS_CREATION_DATE"))
