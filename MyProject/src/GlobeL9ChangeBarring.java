...
import amdocs.uams.UamsException;
import amdocs.uams.UamsSystem;
import amdocs.uams.login.direct.UamsDirectLoginContext;
import amdocs.uamsimpl.client.login.direct.CredDirectLoginService;
import amdocs.uamsx.login.ejb.basic.UamsEjbBasicLogin;
import amdocs.uamsx.login.ejb.basic.UamsEjbBasicLoginHome;

public class GlobeL9ChangeBarring {

	private static final String WL_MACHINE_PORT = "indlnqw253:34100";
	private static final String URL = "t3://indlnqw253:34100";	
	private static final String user = "omsproxy";
	private static final String appId = "OMS";
	...
	public static void main(String[] args)
	{
		...
		GlobeL9ChangeBarring obj = new GlobeL9ChangeBarring();
		InitialContext initCtx = obj.getInitialContext(user, password, appId, WL_MACHINE_PORT);
		//InitialContext initCtx = obj.getInitialContext(user, "LE_Env_Oms");
	...
	private InitialContext getInitialContext(String mctcUser, String mctcEnv) 
	{
		InitialContext ctx = null;
		UamsDirectLoginContext uamsLoginCtx = null;
		String securityRoot = "C:\\Himanshu\\EclipseWorkspace\\MyProject";
		
		try {
			CredDirectLoginService svc = (CredDirectLoginService) UamsSystem.getService(URL, UamsSystem.LN_UAMS_CRED_LOGIN);
			System.out.println("@@@After CredDirectLoginService"+svc);
			String mctcFilePath = securityRoot + "/" + "uams_ctc.xml";
			System.out.println("the mctcFilePath"+ mctcFilePath);
			File ctcFile = new File(mctcFilePath);   
			System.out.println("mctcUser value is :"+ mctcUser);
			System.out.println("mctcEnv value is :"+ mctcEnv);
			if (ctcFile != null && ctcFile.exists() && ctcFile.canRead()) {
				uamsLoginCtx = svc.login(mctcUser, ctcFile, null, mctcEnv);
			}
